		...
		aTab1.VehicleOptions	= guiCreateGridList ( 0, 0, gx, 200, false )
						  ...
						  for i = 0, 211 do
							if ( getVehicleNameFromID ( 400 + i ) ~= "" ) then
								guiGridListSetItemText ( aTab1.VehicleOptions, guiGridListAddRow ( aTab1.VehicleOptions ), 1, getVehicleNameFromID ( 400 + i ), false, false )
							end
						  end
		aTab2 = {}
		...
		if ( getElementParent ( source ) == aTab1.Tab ) then
			...
			elseif ( getElementType ( source ) == "gui-button" )  then
				...
				if ( guiGridListGetSelectedItem ( aTab1.PlayerList ) == -1 ) then
					...
					if ( source == aTab1.Kick ) then aInputBox ( "Kick nub "..name.." out", "Whai?", "", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..name.."\" ), \"kick\", $value )" )
					elseif ( source == aTab1.Ban ) then aInputBox ( "Bant nub "..name, "Whai?!", "gay!", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..name.."\" ), \"ban\", $value )" )
					...
					elseif ( source == aTab1.Shout ) then aInputBox ( "Shout", "Enter text the nub would see", "", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..name.."\" ), \"shout\", $value )" )
					elseif ( source == aTab1.SetHealth ) then aInputBox ( "Set Health", "Enter the health value", "100", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..name.."\" ), \"sethealth\", $value )" )
					elseif ( source == aTab1.SetArmour ) then aInputBox ( "Set Armour", "Enter the armour value", "100", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..name.."\" ), \"setarmour\", $value )" )
