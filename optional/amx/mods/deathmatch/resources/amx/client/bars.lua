...
addEventHandler( "onClientRender", getRootElement(),
	...
	for k, player in pairs( getElementsByType("player") ) do
		...
		if isElementOnScreen( player ) and getDistanceBetweenPoints2D(px, py, x, y) < 20 and player ~= getLocalPlayer() and getElementHealth(player) > 0 then
	...
	for k, myPed in pairs( getElementsByType("ped") ) do
		...
		if isElementOnScreen( myPed ) and getDistanceBetweenPoints2D(px, py, x, y) < 20 and getElementHealth(myPed) > 0 then
			local X, Y = getScreenFromWorldPosition( x, y, z )
			if X then
	...
	if VehicleBars == true then
		for k, vehicle in pairs( getElementsByType("vehicle") ) do
			...
			if isElementOnScreen( vehicle ) and getDistanceBetweenPoints2D(px, py, x, y) < 20 and vehicle ~= getPedOccupiedVehicle(getLocalPlayer())  and getElementHealth(vehicle) > 0  then
				local X, Y = getScreenFromWorldPosition( x, y, z )
				if X then
