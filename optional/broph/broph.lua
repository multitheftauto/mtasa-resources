...
------------------------------------------------
-- temp stuff for tracking #4010
------------------------------------------------
function skinTestStart(plr)
    outputDebugString( "skinTestStart" );
	if not skinTestTimer then
        skinTestTimer = setTimer( skinTestPulse, 50, 0, plr )
	end
end

function skinTestStop()
    outputDebugString( "skinTestStop" );
	if skinTestTimer then
        killTimer( skinTestTimer )
        skinTestTimer = nil
	end
end

function skinTestPulse(plr)
    for i,player in ipairs(getElementsByType('player')) do
        if player == plr then
            if not skinIDStep or skinIDStep > 287 then
                skinIDStep = 0
            end
            setElementModel(player,skinIDStep)
            skinIDStep = skinIDStep + 1
        end
        return
    end
    skinTestStop()
...
addCommandHandler('steston', skinTestStart)
addCommandHandler('stestoff', skinTestStop)
------------------------------------------------
