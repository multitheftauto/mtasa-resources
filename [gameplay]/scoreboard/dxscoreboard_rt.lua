...
function drawScoreboard()
	...
	if not scoreboardDrawn then
		doDrawScoreboard ( false--[[rtPass]], true--[[onlyAnim]], SCREEN_X, SCREEN_Y )
		bForceUpdate = true
		return
	end
	...
	if not rt or getKeyState("num_9") then
		doDrawScoreboard ( false --[[rtPass]], false --[[onlyAnim]], SCREEN_X, SCREEN_Y )
		return
	end
	...
	if newRowIdx ~= lastRowIdx then
		...
		bUpdate = true -- -- Force rt update if highlighted row will change
	end

	-- Draw background to screen

	doDrawScoreboard ( true --[[rtPass]], true --[[onlyAnim]], SCREEN_X, SCREEN_Y )
	...
	if bUpdate then
		...
		doDrawScoreboard ( true --[[rtPass]], false --[[onlyAnim]], sX, sY )
	...
	local x, y = scoreboardGetTopCornerPosition()
	dxSetBlendMode( "add" )
	dxDrawImage( x, y, sX, sY, rt, 0, 0, 0, cWhite, drawOverGUI )
	dxSetBlendMode( "blend" )
end
...
function checkCursorOverRow(rtPass, xl,xh,yl,yh)
	...
	local cursorOverRow = cX >= xl and cX <= xh and cY >= yl and cY <= yh

	savedRowsNext[#savedRowsNext + 1] = {xl, xh, yl, yh}

	return cursorOverRow
end
