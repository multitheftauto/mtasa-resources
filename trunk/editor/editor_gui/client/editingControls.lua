...
eC.vehiclecolors = {
	...
	constructor = function( self, info )
		local paddingX = info.paddingX
		if info.relative then
			paddingX = paddingX / (guiGetScreenSize())
		end
		
		self.boxwidth = (info.width / 4) - paddingX * 2
		self.boxheight = info.height
		self.relative = info.relative
		self.rows = info.rows
		...
		for i = 1, 4 do
			self.GUI["imgColorBorder"..i] = guiCreateStaticImage(
				info.x + self.boxwidth * (i-1) + paddingX * (i-1),
				info.y,
				self.boxwidth - paddingX,
				self.boxheight,
				"client/images/vcolors/vcolor_1.png", --!
				self.relative,
				self.parent
			)
			guiSetAlpha(self.GUI["imgColorBorder"..i], .8)
			
			self.GUI["imgColor"..i] = guiCreateStaticImage(
				info.border,
				info.border,
				self.boxwidth - paddingX - info.border*2,
				self.boxheight - info.border*2,
				"client/images/vcolors/vcolor_0.png",
				self.relative,
				self.GUI["imgColorBorder"..i]
			)
		end
		
		self.WGUI.coverWindow = guiCreateWindow(0,0,1,1,"",true)
		guiSetAlpha(self.WGUI.coverWindow, 0)
		guiSetVisible(self.WGUI.coverWindow, false)
		
		local windowWidth = self.boxwidth * (self.columns+1)
		local windowHeight = self.boxheight * (self.rows+2)
		self.WGUI.selectWindow = guiCreateWindow(
			windowWidth / 2,
			windowHeight / 2,
			windowWidth,
			windowHeight,
		...
		for i = 1, self.rows do
			for k = 1, self.columns do
				...
				self.WGUI["imgListColor"..n] = guiCreateStaticImage(
					self.boxwidth * (k-.5), --!fixme
					self.boxheight * i,
					self.boxwidth,
					self.boxheight,
					"client/images/vcolors/vcolor_"..n..".png",
					self.relative,
					self.WGUI.selectWindow
				)
		...
		self:addHandler("onClientGUIClick", self.parent, self.showColorList, true)
		
		return self
	end,
	setValue = function( self, value ) --!add checking
		for i = 1, 4 do
			guiStaticImageLoadImage(
				self.GUI["imgColor"..i],
				"client/images/vcolors/vcolor_"..value[i]..".png"
			)
			guiBringToFront(self.GUI["imgColor"..i])
		end
	...
	getValue = function( self )
		return table.copy(self.value)
	end,
	showColorList = function( self, passedSource )
		if type(passedSource) ~= "string" then source = passedSource end
		local clickedColor
		for i = 1, 4 do
			if self.GUI["imgColor"..i] == source then
				clickedColor = i
				break
			elseif self.GUI["imgColorBorder"..i] == source then
				clickedColor = i
				guiBringToFront(self.GUI["imgColor"..i])
				break
			end
		end
		if not clickedColor then
			return false
		end
		
		self.selecting = clickedColor
		
		guiSetVisible(self.WGUI.coverWindow, true)
		guiSetVisible(self.WGUI.selectWindow, true)
		
		guiBringToFront(self.WGUI.coverWindow)
		guiBringToFront(self.WGUI.selectWindow)
		
		--don't let the cover get on top of the selection window
		self:addHandler("onClientGUIClick", self.WGUI.coverWindow,
			function()
				guiBringToFront(self.WGUI.selectWindow)
			end
		)
		self:addHandler("onClientGUIClick", self.WGUI.selectWindow, self.selectColour, true)
	end,
	selectColour = function( self )
		--ignore clicks on the window itself
		if source == this then
			return
		end
		...
		local clickedColor
		for n = 0, 126 do
			if source == self.WGUI["imgListColor"..n] then
				clickedColor = n
				break
			end
		end
		...
		newValue[self.selecting] = clickedColor
		self:setValue(newValue)
		
		self.selecting = nil
		
		self:removeHandler() --self.GUI.selectWindow...
		self:removeHandler() --self.GUI.coverWindow...
		guiSetVisible(self.WGUI.selectWindow, false)
		guiSetVisible(self.WGUI.coverWindow, false)
	end,
	focus = function(self)
		setCursorPosition ( guiElementGetScreenPosition(self.WGUI.selectWindow) )
		self:showColorList(self.GUI["imgColorBorder1"])
	end,
}
eC.vehiclecolours = eC.vehiclecolors
