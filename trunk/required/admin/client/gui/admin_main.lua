		...
		if ( getElementParent ( source ) == aTab1.Tab ) then
			...
			elseif ( getElementType ( source ) == "gui-button" )  then
				...
				if ( guiGridListGetSelectedItem ( aTab1.PlayerList ) == -1 ) then
					...
					elseif ( source == aTab1.Ban ) then aInputBox ( "Ban player "..name, "Enter the ban reason (This works now)", "", "triggerServerEvent ( \"aPlayer\", getLocalPlayer(), getPlayerFromNick ( \""..escname.."\" ), \"ban\", $value )" )
