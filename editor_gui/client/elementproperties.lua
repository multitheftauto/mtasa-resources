...
pulloutAction.CopyPosRot = function()
    if not selectedElement then return end
    local x, y, z = edf.edfGetElementPosition(selectedElement)
    local rx, ry, rz = edf.edfGetElementRotation(selectedElement)
    setClipboard(string.format("%s, %s, %s, %s, %s, %s", tostring(x), tostring(y), tostring(z), tostring(rx), tostring(ry), tostring(rz)))
    outputMessage("Copied position and rotation: " .. x .. ", " .. y .. ", " .. z .. ", " .. rx .. ", " .. ry .. ", " .. rz)
end
