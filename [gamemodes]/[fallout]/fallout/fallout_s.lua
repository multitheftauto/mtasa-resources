...
function resourceStart()
	call(getResourceFromName("scoreboard"), "addScoreboardColumn", "Rank", root, 2, .08)
...
function createBoard() --MOVE BOARD CREATION TO CLIENTSIDE
	...
	for i = 1, rows do --Nested to create rows and columns
		for j = 1, columns do
			board[count] =
				createObject(1697, 1540.122926 + 4.466064 * j, -1317.568237 + 5.362793 * i, 603.105469, math.deg(0.555), 0, 0)
			count = count + 1
		end
	end
end
...
function playerLoss()
	...
	textDisplayAddObserver(suicideDisplay, client)
...
addEvent("serverReportLoss", true)
...
addEventHandler("serverClientLoad", root, clientLoaded)