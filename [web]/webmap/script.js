...
function init() {
				...
				layers: [
					new ol.layer.Tile({
						source: new ol.source.XYZ({
							projection: gtasaProjection,
							tileSize: 500,
							url: 'https://assets.mtasa.com/mtasa-resources/webmap/sa_aerial_map/v2/{z}_{x}_{y}.jpg',
							wrapX: false,
						}),
					...
					new ol.layer.Tile({
						source: new ol.source.XYZ({
							projection: gtasaProjection,
							tileSize: 500,
							url: 'https://assets.mtasa.com/mtasa-resources/webmap/sa_aerial_map/v1/{z}_{x}_{y}.jpg',
							wrapX: false,
						}),
					...
					new ol.layer.Tile({
						source: new ol.source.XYZ({
							projection: gtasaProjection,
							tileSize: 500,
							url: 'https://assets.mtasa.com/mtasa-resources/webmap/sa_map/v1/{z}_{x}_{y}.jpg',
							wrapX: false,
						}),
	...
	map.on('singleclick', function (e) {
		...
		for (let i = 0; i < features.length; i++) {
			...
			let html = selectedPlayer + "<button type='button' onclick='closePopup();' style='position:absolute;right:0;top:0;'>close</button><br/><div style='font-size: 0.8em;'>";
	
			if (player.vehicle)
				html += "In vehicle: " + player.vehicle + "<br/>";
...
function updateRadarBlips() {
	getAllRadarBlips(function (blips) {
		...
		Object.entries(syncedRadarBlips).filter(function (entry) {
		...
		blips.forEach(function (blip) {
			if (!syncedRadarBlips[blip.element.id]) {
				...
				radarBlipVectorSource.removeFeature(syncedRadarBlips[blip.element.id].feature);
			}
...
function updatePlayerBlips() {
	getAllPlayers(function (players) {
		...
		Object.entries(syncedPlayers).filter(function (entry) {
		...
		if (selectedPlayer) {
			if (syncedPlayers[selectedPlayer]) {
				popupOverlay.setPosition(syncedPlayers[selectedPlayer].feature.getGeometry().getCoordinates());
			} else {
		...
		if (followingPlayer) {
			if (syncedPlayers[followingPlayer]) {
				map.getView().setCenter(syncedPlayers[followingPlayer].feature.getGeometry().getCoordinates());
			} else {
