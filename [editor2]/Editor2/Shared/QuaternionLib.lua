------------------------------------------------------------------------------------------------
---Credit || Deltanic, for the quaternion calculations in order to pull off proper rotations----
------------------------------------------------------------------------------------------------
...
function Quaternion:toAngle()
	...
	local x = Vector3(q1*q1 + q2*q2 - q3*q3 - q4*q4,
		2*q3*q2 + 2*q4*q1,
		2*q4*q2 - 2*q3*q1)
		
	local y = Vector3(2*q2*q3 - 2*q4*q1,
...
function quat(angle, vec)
  ...
  return quat_new(w, s * vec.x, s * vec.y, s * vec.z)
end
